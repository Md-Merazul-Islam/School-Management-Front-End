  ...
  const handleSearch = async (e) => {
    ...
    try {
      const response = await fetch(
        `https://school-management-dusky.vercel.app/academics/students-list/?first_name=${firstName}&username=${username}&roll_no=${rollNo}`
      );
      ...
      const updatedStudents = fetchedStudents.map((student) => {
        if (student.marksheet.length === 0) {
          return {
            ...student,
            status: "No marksheet available",
            final_gpa: null,
            final_grade: null,
          };
        }

        const failedSubject = student.marksheet.some(
              ...
              {student.final_gpa && student.final_grade ? (
                <div className="mt-4">
                  <p className="text-lg font-semibold">
                    Final GPA: {student.final_gpa} <span>(Out Of 4)</span>{" "}
                    <br /> Final Grade: {student.final_grade}
                  </p>
                </div>
              ) : (
                <p className="text-gray-500 mt-4">
                  No GPA or grade data available
                </p>
              )}
            </div>
          ))}
